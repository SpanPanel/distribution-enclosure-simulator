...
- **The README's device-metadata table named five keys that did not match the parser, in both directions.** That table is the contract a producer builds a `DeviceManifest` from, and since 0.3.2 it is also the PyPI project page. Two documented keys did not exist, so following the table raised: `pv`'s `nameplate-capacity-w` (the parser reads `nominal-power-w`, which is what the spec directs power-rated devices to publish) and `evse`'s `product-name` (reads `model`). Three more were read and undocumented, so they could not be set and the resulting properties were simply absent from the tree with no error anywhere: `panel`'s `schema-topology`, `circuit`'s `dipole`, `bess`'s `part-number`. Reported by [@cayossarian](https://github.com/cayossarian) in [#23](https://github.com/electrification-bus/distribution-enclosure-simulator/issues/23).
...
- **The `feed-circuit-id` metadata key, in favour of `feed`.** The original name from the first commit, superseded and unused by anything in `src/` or `examples/`. It still resolves, so no existing manifest breaks, but it now raises `DeprecationWarning` whenever it is present — including when `feed` is also given and wins, since a producer part-way through migrating is exactly who needs to hear that the old key has become dead weight. It stays out of the README deliberately: documenting it would entrench two names for one concept.
