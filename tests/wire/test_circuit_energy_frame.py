...
from ebus_panel_sim.wire.bag_builder import _RESOLVERS
...
def test_load_circuit_exports_energy_and_reads_negative_power() -> None:
    ...
    snap = _snapshot(
        _circuit(instant_power_w=121.0, consumed_energy_wh=2.0166, produced_energy_wh=0.0)
    )

    assert _ACTIVE_POWER(snap, "c1") == -121.0
    assert _EXPORTED(snap, "c1") == 2.0166
...
def test_backfeeding_circuit_imports_energy_and_reads_positive_power() -> None:
    ...
    snap = _snapshot(
        _circuit(instant_power_w=-8500.0, consumed_energy_wh=0.0, produced_energy_wh=141.6666)
    )

    assert _ACTIVE_POWER(snap, "c1") == 8500.0
    assert _IMPORTED(snap, "c1") == 141.6666
...
def test_resolvers_return_none_for_unknown_circuit() -> None:
    snap = _snapshot(
        _circuit(instant_power_w=121.0, consumed_energy_wh=1.0, produced_energy_wh=0.0)
    )
