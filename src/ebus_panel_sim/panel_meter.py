...
The two frames disagree about the same instant, on purpose. A panel exporting
2.5 kW publishes ``lugs-upstream/active-power`` negative and ``power-flows/grid``
positive simultaneously; both are correct, because they answer different
questions. Do not "make them consistent".

The balance is what makes the node frame checkable: hardware's four flows sum to
zero to the last digit it publishes. ``test_power_flows_sum_to_zero`` holds this
emitter to the same identity, which is why ``power_flow_grid`` is derived from
the physics below rather than back-solved from the other three — a residual
would satisfy the test by construction and detect nothing.
